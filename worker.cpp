...
void WorkClass::process()
{
    ...
    //Set Data Format:
    if(!Abort()) this->Osci.write("STOP","");
    if(!Abort()) this->Osci.write("FORM:DATA INT,16","");
    if(!Abort()) this->Osci.write("EXP:WAV:INCX OFF","");
    ...
    while(!Abort())
    {
        ...
        if(this->m_data.find(DeviceName + "::Aquisition::Count") != this->m_data.end())
        {
            double AquisitionCounter = this->m_data[DeviceName + "::Aquisition::Count"].GetDouble();
            if( AquisitionCounter > LastAquisitionCounter)
            {
                LastAquisitionCounter = AquisitionCounter;
                if(this->m_data[DeviceName + "::Trigger::Run"].GetString() == "RUNSingle")
                {
                    this->m_data[DeviceName + "::Trigger::Run"].SetData("STOP");
